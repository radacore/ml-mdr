...
export default function Create() {
    ...
    // Auto determine Usia category
    const determineUsiaCategory = (age: number) => {
        if (age >= 45) return 'Usia Lanjut';
    ...
    const handleAgeChange = (value: string) => {
        ...
        if (!isNaN(age)) {
            setData('usia', determineUsiaCategory(age));
        }
    };

    const submit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        post(route('training-data.store'));
    };

    return (
        <AppLayout breadcrumbs={[{ label: 'Data Training', href: '/training-data' }, { label: 'Tambah' }]}>
            <Head title="Tambah Data Training" />

            <Card>
                <CardHeader>
                    <CardTitle>Form Data Pasien</CardTitle>
                    <CardDescription>
                        Masukkan data pasien dengan hasil pengobatan yang sudah diketahui
                    </CardDescription>
                </CardHeader>
                <CardContent>
